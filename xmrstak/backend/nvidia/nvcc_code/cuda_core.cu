...
void cryptonight_core_cpu_hash(nvid_ctx* ctx, xmrstak_algo miner_algo, uint32_t startNonce)
{
	...
	else if(miner_algo == cryptonight_alloy)
	{
		...
		else if(miner_algo == cryptonight_marketcash)
	{
		cryptonight_core_gpu_hash<CRYPTONIGHT_MARKETCASH_ITER, CRYPTONIGHT_MARKETCASH_MASK, CRYPTONIGHT_MARKETCASH_MEMORY/4, cryptonight_marketcash>(ctx, startNonce);
	}
}
